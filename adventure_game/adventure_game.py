...
#function to add money to player inventory
def give_money(amount):
    save['Inventory']['Coins'] = save['Inventory']['Coins'] + amount
...
def battle(enemy):
    ...
    while True:
        ...
        if enemy['Health'] > 0:    
            ...
            #Give player money and exit the battle
            give_money(enemy['Reward'])
...
def a3():
    while True:
        ...
        if player_input == 'fight' or player_input == 'battle':
            ...
            #the below line would be for an alternate save system that saves as you leaves rooms
            #and reloads your location using the while True loop without the break in it
            #this resolves a bug where reloading the game when not in the first room
            #makes it so you cant leave any rooms without quitting the game
            #this system would require more rewritting and has been abandoned in favor
            #of always making you start in the first room...because im lazy
